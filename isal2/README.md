...
当前完成第一、二阶段：RPO 机器人资源、完整 base 环境、MLP/PPO、任务列表及训练脚本。AME、affordance、ONNX 导出和 MuJoCo 手柄控制尚未实现，其目录预留给后续阶段。
...
`modified_rsl` 中的 PPO 和 runner 继承外部对应类，storage、MLP、归一化基础组件与日志工具直接复用外部库。后续 AME/affordance 网络写在 `modified_rsl/modules`。
