...
The robot articulation and URDF remain read-only references to `robolab.assets.robots.RPO_CFG`; a namespace-only fallback supports direct execution from the monorepo root. No robot assets are duplicated into this project.

The existing BSD-3-Clause copyright and license notices are retained in copied source files.

Robot configuration and URDF/data assets are not copied. They are referenced read-only through `RPO_CFG`.
